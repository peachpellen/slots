...
//Rare exceptions to indicate a bug in the _slots code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_slots = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_slots]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.slots.module"
